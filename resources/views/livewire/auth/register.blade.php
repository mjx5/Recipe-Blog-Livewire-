                    ...
                    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form class="space-y-4 md:space-y-6" wire:submit.prevent="register">
                            ...
                            <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                                Already have an account? <a wire:navigate href="{{ url('login') }}" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Login here</a>
                            </p>
                            @if (session()->has('message'))
                                <span class="text-green-500 text-xs">{{ session('message') }}</span>
                            @endif
                        </form>
                        <button wire:click="dumpValues" class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">dump values</button>
                    </div>
                </div>
            </div>
        </section>
